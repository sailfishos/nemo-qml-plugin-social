...
void displayUsage()
{
    ...
    qWarning() << "       twittersocialtest [consumer_key] [consumer_secret]";
}
...
TokenRequestHandler::TokenRequestHandler(Data *data, QObject *parent)
    ...
    , m_startRequestReply(0), m_continueRequestReply(0)
{
}

QString TokenRequestHandler::pin() const
{
    return m_pin;
}

void TokenRequestHandler::setPin(const QString &pin)
{
    if (m_pin != pin) {
        m_pin = pin;
        emit pinChanged();
...
{
    if (m_data->token.isEmpty() || m_pin.isEmpty()) {
        return;
    }
...
int main(int argc, char *argv[])
{
    ...
    Data data;
    bool request = (arguments.count() == 3);
    if (request) {
        data.appId = arguments.at(1).toLatin1();
        data.appSecret = arguments.at(2).toLatin1();
        ...
        qDebug() << "Consumer key:" << arguments.at(1);
        qDebug() << "Consumer secret:" << arguments.at(2);
        qDebug() << "Token:" << arguments.at(3);
        qDebug() << "Token secret:" << arguments.at(4);
